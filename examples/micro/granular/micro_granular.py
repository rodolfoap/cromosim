...
from cromosim.micro import projection, move_people, people_update_destination
...
print("===> Minimal distance between persons, dmin_people = ", dmin_people)
print("===> Minimal distance between a person and a wall, dmin_walls = ", dmin_walls)
...
for i, peopledom in enumerate(json_people_init):
    ...
    II, JJ, Vd = dom.people_desired_velocity(people["xyrv"], people["destinations"])
    people["Vd"] = Vd
...
while (t < Tf):
    ...
    for idom, name in enumerate(domains):
        ...
        II, JJ, Vd = dom.people_desired_velocity(
            people["xyrv"],
            people["destinations"])
        people["Vd"] = Vd
        people["I"] = II
